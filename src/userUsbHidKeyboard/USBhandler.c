...
// clang-format off
// v2.3.1 と同じバッファサイズ (usbcdc 148B で足りる)
__xdata __at (EP0_ADDR) uint8_t Ep0Buffer[0];     // EP1 と共有
__xdata __at (EP1_ADDR) uint8_t Ep1Buffer[72];    // keyboard: OUT 0..63, IN 64..71
__xdata __at (EP2_ADDR) uint8_t Ep2Buffer[72];    // gamepad:  OUT 0..63, IN 64..71
// clang-format on

// EP0[0] + EP1[72] + EP2[72] = 144B — usbcdc (148B) で足りる

// Gamepad TX busy flag (parallel to UpPoint1_Busy for keyboard)
    ...
    } else // Standard request
    {
      ...
      case USB_GET_DESCRIPTOR:
        ...
        case 3:
          ...
          break;
        case 0x22:
          // Dispatch report descriptor by interface index (wIndexL)
          if (UsbSetupBuf->wIndexL == 0) {
            pDescr = (__code uint8_t *)GamepadReportDescriptor;
            len = ConfigurationDescriptor.Gamepad_HID.HIDReportLength;
          } else if (UsbSetupBuf->wIndexL == 1) {
            pDescr = (__code uint8_t *)KeyboardReportDescriptor;
            len = ConfigurationDescriptor.Keyboard_HID.HIDReportLength;
          } else {
            len = 0xff;
          }
          break;
...
void USBDeviceEndPointCfg() {
  ...
  UEP1_DMA = (uint16_t)Ep1Buffer;  // keyboard EP1
  UEP2_DMA = (uint16_t)Ep2Buffer;  // gamepad  EP2

  UEP1_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK | UEP_R_RES_ACK;  // EP1 keyboard
  UEP2_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK | UEP_R_RES_ACK;  // EP2 gamepad
  UEP4_1_MOD = 0xC0;  // EP1 TX+RX enable (bit7=RX, bit6=TX for EP1)
  UEP2_3_MOD = 0x0C;  // EP2 TX+RX enable (bit3=RX, bit2=TX for EP2) — FIX: was 0xC0 which enabled EP3 not EP2
  UEP0_CTRL = UEP_R_RES_ACK | UEP_T_RES_NAK;
}
